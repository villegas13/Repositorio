...
{ 
 ...
 chat_rem = document.getElementById('chat_rem').value;
         ...
         $("#textoprivsetpopup").load('<?php echo $pluing_url."/cargarpriv.php"; ?>',{texto:texto_chat,chat_user:chat_user,chat_rem:chat_rem,chat_des:chat_des},  function(response, status, xhr) {;
              if (status == "error") {
                var msg = "Error!, algo ha sucedido: ";
                $("#textoprivsetpopup").html(msg + xhr.status + " " + xhr.statusText);
              }
...
{ 
 ...
 $(document).ready(function() { 
             ...
             $("#textoprivsetpopup").load("<?php echo plugins_url( 'chatpriv.php', __FILE__ );?>", {texto:texto_chat,chat_user:chat_user,chat_rem:chat_rem,chat_des:chat_des}, function(response, status, xhr) {
              if (status == "error") {
                var msg = "Error!, algo ha sucedido: ";
                $("#textoprivsetpopup").html(msg + xhr.status + " " + xhr.statusText);
              }
...
function borrar(id){
 ...
 chat_id = id;
         ...
         $("#textoprivsetpopup").load('<?php echo $pluing_url."/borrar.php"; ?>',{chat_id:chat_id},  function(response, status, xhr) {;
              if (status == "error") {
                var msg = "Error!, algo ha sucedido: ";
                $("#textoprivsetpopup").html(msg + xhr.status + " " + xhr.statusText);
              }
...
<i><span class="titulo2"><?php echo "&nbsp;&nbsp;&nbsp;&nbsp;".$user;?></span></i>

<div id="resultadoprivsetpopup"> 


<div id="textoprivsetpopup"> 
...
<hr>
<div id="botonesprivsetpopup"> 
 
<label id="mensaje"><br> 
<input  type="text" id="texto_chat" placeholder="Escribe un mensaje aqu&iacute" name="texto_chat" autocomplete="off" style="width:200px">
<?php
  echo '<a href="popupsmiley.php" onclick="return popitup2(\''. $pluing_url.'popupsmiley.php' .'\')" class="link" ><img title="Agregar Smiley" src="'. get_stylesheet_directory_uri().'/../../plugins/sala-de-chat/images/emojs/smile.gif' .'" ></a>&nbsp;&nbsp;';
?>
</label>
<input type="button" id="enviar" name="enviar" value="Responder"  onclick="chatear()" > <br><br>
<label id="usuario" style="display:none">Usuario: <input type="hideen" id="chat_user" name="chat_user" value=<?php if (is_user_logged_in()){ $cu = wp_get_current_user(); echo $cu->user_login;} else{echo $_COOKIE['invitado'];} ?> > </label>
